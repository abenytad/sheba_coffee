...
const ContactForm: React.FC = () => {
  ...
  const isValidPhone = (phone: string) => /^\+?[1-9]\d{1,14}$/.test(phone); // Basic international phone number validation
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // If there are errors, they will be displayed below each input field
  };
    ...
    <div className="flex-1 p-4 md:p-6 lg:p-8 bg-white rounded-lg shadow-md">
      ...
      <form className="flex flex-col" onSubmit={handleSubmit}>
        ...
        <input
          ...
          onChange={handleChange}
          className={`border ${errors.phone ? 'border-red-500' : 'border-gray-300'} rounded p-3 mb-4 transition-all duration-300 ease-in-out focus:ring focus:ring-primary focus:border-transparent`}
        />
